...
LOGIN_USERNAME, LOGIN_PASSWORD = range(10, 12)
(PROD_NAME, PROD_CATEGORY, PROD_BUY, PROD_SELL, PROD_STOCK,
 PROD_MIN, PROD_IMAGE, PROD_SIZES, PROD_COLORS, PROD_PRINTS,
 PROD_NOTES) = range(12, 23)
(CUST_NAME, CUST_PHONE, CUST_ADDR) = range(23, 26)
EDIT_VALUE = 30

# ── Helpers ──────────────────────────────────────────

def get_db(context):
    db = context.user_data.get("_db")
    if db is None:
        db = SessionLocal()
        context.user_data["_db"] = db
    return db

def close_db(context):
    db = context.user_data.pop("_db", None)
    if db:
        db.close()

def clean_cart(context):
    for k in ["cart", "customer_name", "checkout_step", "pos_cat", "pos_page", "pos_pid", "pos_qty"]:
        context.user_data.pop(k, None)

async def auth_gate(update, query, context):
...
async def login_password(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user == "za_c10" and pw == "1942":
        ...
        await update.message.reply_text("❌ اسم المستخدم أو كلمة السر خطأ.\n👤 أدخل اسم المستخدم مرة أخرى:")
        context.user_data.pop("login_user", None)
        return LOGIN_USERNAME
...
async def login_cancel(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.answer()
    context.user_data.clear()
    await query.edit_message_text("❌ تم إلغاء تسجيل الدخول.")
    return ConversationHandler.END
...
async def pos_categories(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    if not await auth_gate(update, query, context): return
    db = get_db(context)
...
async def pos_products(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("cat_"):
        context.user_data["pos_cat"] = int(data.split("_")[1])
...
async def pos_product_select(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if sizes: text += f"المقاسات: {', '.join(sizes)}\n"
    if colors: text += f"الألوان: {', '.join(colors)}\n"
    if prints: text += f"الطباعة: {', '.join(prints)}\n"
    ...
    if img_path and img_path.exists():
        ...
        await query.edit_message_text(text + "\nالكمية: 1", reply_markup=qty_keyboard(pid))
...
async def pos_qty_change(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    qty = qty + 1 if parts[1] == "up" else max(1, qty - 1)
    db = get_db(context)
    p = db.query(Product).filter(Product.id == pid).first()
...
async def pos_add_to_cart(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for item in cart:
        ...
        cart.append({"pid": pid, "name": p.name, "price": p.sell_price, "qty": qty, "total": p.sell_price * qty})
...
async def pos_checkout(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    cart = context.user_data.get("cart", [])
    if not cart:
        await pos_view_cart(update, context); return
    total = sum(item["total"] for item in cart)
    text = "🧾 *تأكيد الفاتورة*\n\n"
    for item in cart:
        text += f"• {item['name']} × {item['qty']} = {item['total']:,.0f} د.ع\n"
    text += f"\n*الإجمالي: {total:,.0f} د.ع*\n\nالرجاء إرسال اسم العميل (أو اختر أدناه):"
...
async def pos_confirm_invoice(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    invoice = Invoice(
        ...
        customer_name=customer_name, total=total, grand_total=total,
        invoice_date=date.today(),
    )
    db.add(invoice); db.flush()
    for item in cart:
        db.add(InvoiceItem(invoice_id=invoice.id, product_id=item["pid"],
        ...
        if prod:
            ...
            db.add(StockMovement(product_id=prod.id, type="out",
                quantity=-item["qty"], reference=invoice.invoice_number))
    db.commit()
    clean_cart(context)
    from bot.keyboards import back_to_menu
    text = f"✅ *تم حفظ الفاتورة!*\n\nرقم: #{invoice.invoice_number}\nالعميل: {customer_name}\nالإجمالي: {total:,.0f} د.ع"
...
async def products_list(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    if not await auth_gate(update, query, context): return
    page = int(query.data.split("_")[1]) if query.data.startswith("prods_") else 0
    db = get_db(context)
    ...
    await query.edit_message_text(f"📦 *المنتجات* (صفحة {page+1}/{total_pages})",
        reply_markup=products_list_keyboard(products[start:start+PER_PAGE], page, total_pages))
...
async def product_view(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    p = db.query(Product).filter(Product.id == pid).first()
    if not p:
        await query.edit_message_text("المنتج غير موجود."); return
    ...
    if img_path and img_path.exists():
        with open(img_path, "rb") as f:
            ...
            await context.bot.send_photo(chat_id=query.message.chat_id, photo=f,
                caption=text, reply_markup=product_view_keyboard(pid))
    else:
...
async def prod_add_buy(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try: context.user_data["new_prod"]["buy_price"] = float(update.message.text)
    except ValueError:
        await update.message.reply_text("الرجاء إرسال رقم صحيح:"); return PROD_BUY
    from bot.keyboards import add_prod_cancel
...
async def prod_add_sell(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try: context.user_data["new_prod"]["sell_price"] = float(update.message.text)
    except ValueError:
        await update.message.reply_text("الرجاء إرسال رقم صحيح:"); return PROD_SELL
    from bot.keyboards import add_prod_cancel
...
async def prod_add_stock(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try: context.user_data["new_prod"]["stock"] = int(update.message.text)
    except ValueError:
        await update.message.reply_text("الرجاء إرسال رقم صحيح:"); return PROD_STOCK
    from bot.keyboards import add_prod_cancel
...
async def prod_add_min(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try: context.user_data["new_prod"]["min_stock"] = int(update.message.text)
    except ValueError:
        await update.message.reply_text("الرجاء إرسال رقم صحيح:"); return PROD_MIN
    from bot.keyboards import skip_btn
    await update.message.reply_text("أرسل صورة المنتج (أو اختر تخطي):", reply_markup=skip_btn("skip_image"))
    return PROD_IMAGE
...
async def prod_add_image(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if update.message and update.message.photo:
        ...
        fname = f"prod_{datetime.now().strftime('%Y%m%d%H%M%S%f')}.jpg"
        await file.download_to_drive(PRODUCT_IMAGES_DIR / fname)
        context.user_data["new_prod"]["image"] = fname
    else: context.user_data["new_prod"]["image"] = ""
    from bot.keyboards import skip_btn
    await update.message.reply_text("أرسل المقاسات (مفصولة بفاصلة) أو اختر تخطي:", reply_markup=skip_btn("skip_sizes"))
    return PROD_SIZES
...
async def prod_add_image_skip(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    from bot.keyboards import skip_btn
    await query.edit_message_text("أرسل المقاسات (مفصولة بفاصلة) أو اختر تخطي:", reply_markup=skip_btn("skip_sizes"))
    return PROD_SIZES
...
async def prod_add_sizes(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    from bot.keyboards import skip_btn
    await update.message.reply_text("أرسل الألوان (مفصولة بفاصلة) أو اختر تخطي:", reply_markup=skip_btn("skip_colors"))
    return PROD_COLORS
...
async def prod_add_sizes_skip(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    from bot.keyboards import skip_btn
    await query.edit_message_text("أرسل الألوان (مفصولة بفاصلة) أو اختر تخطي:", reply_markup=skip_btn("skip_colors"))
    return PROD_COLORS
...
async def prod_add_colors(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    from bot.keyboards import skip_btn
    await update.message.reply_text("أرسل أماكن الطباعة (مفصولة بفاصلة) أو اختر تخطي:", reply_markup=skip_btn("skip_prints"))
    return PROD_PRINTS
...
async def prod_add_colors_skip(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    from bot.keyboards import skip_btn
    await query.edit_message_text("أرسل أماكن الطباعة (مفصولة بفاصلة) أو اختر تخطي:", reply_markup=skip_btn("skip_prints"))
    return PROD_PRINTS
...
async def prod_add_notes(update: Update, context: ContextTypes.DEFAULT_TYPE):
    context.user_data["new_prod"]["notes"] = update.message.text if update.message.text != "/skip" else ""
    await prod_add_save(update, context); return ConversationHandler.END
...
async def prod_add_save(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    p = Product(name=d.get("name",""), category_id=d.get("cat_id"),
        buy_price=d.get("buy_price",0), sell_price=d.get("sell_price",0),
        current_stock=d.get("stock",0), min_stock=d.get("min_stock",5),
        image=d.get("image",""), sizes=d.get("sizes","[]"),
        colors=d.get("colors","[]"), print_locations=d.get("prints","[]"),
        notes=d.get("notes",""))
    db = get_db(context); db.add(p)
    if d.get("stock", 0) > 0:
        db.add(StockMovement(product=p, type="in", quantity=d["stock"],
            reference="telegram", notes="إضافة عن طريق البوت"))
    db.commit(); context.user_data.pop("new_prod", None)
    from bot.keyboards import back_to_menu
...
async def invoices_list(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    if not await auth_gate(update, query, context): return
    page = int(query.data.split("_")[1]) if query.data.startswith("invs_") else 0
    db = get_db(context)
    invoices = db.query(Invoice).order_by(Invoice.id.desc()).all()
    total_pages = max(1, (len(invoices) + PER_PAGE - 1) // PER_PAGE)
    page = min(page, total_pages - 1); start = page * PER_PAGE
    from bot.keyboards import invoices_keyboard
    await query.edit_message_text(f"📋 *الفواتير* (صفحة {page+1}/{total_pages})",
        reply_markup=invoices_keyboard(invoices[start:start+PER_PAGE], page, total_pages))
...
async def invoice_detail(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not inv: await query.edit_message_text("الفاتورة غير موجودة."); return
    items_text = "".join(f"• {i.product_name} × {i.quantity} = {i.total:,.0f} د.ع\n" for i in inv.items)
    text = (f"🧾 *فاتورة #{inv.invoice_number}*\nالتاريخ: {inv.invoice_date}\n"
            f"العميل: {inv.customer_name}\nرقم الهاتف: {inv.customer_phone or '-'}\n\n"
            f"{items_text}\n*الإجمالي: {inv.total:,.0f} د.ع*\n"
            f"الخصم: {inv.discount:,.0f}\nالضريبة: {inv.tax:,.0f}\n*الصافي: {inv.grand_total:,.0f} د.ع*")
...
async def customers_list(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    if not await auth_gate(update, query, context): return
    page = int(query.data.split("_")[1]) if query.data.startswith("custs_") else 0
    db = get_db(context)
    customers = db.query(Customer).order_by(Customer.id).all()
    total_pages = max(1, (len(customers) + PER_PAGE - 1) // PER_PAGE)
    page = min(page, total_pages - 1); start = page * PER_PAGE
    from bot.keyboards import customers_keyboard
    await query.edit_message_text(f"👤 *العملاء* (صفحة {page+1}/{total_pages})",
        reply_markup=customers_keyboard(customers[start:start+PER_PAGE], page, total_pages))
...
async def customer_detail(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not c: await query.edit_message_text("العميل غير موجود."); return
...
async def cust_add_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not await auth_gate(update, query, context): return
    from bot.keyboards import add_prod_cancel
    await query.edit_message_text("أرسل اسم العميل:", reply_markup=add_prod_cancel()); return CUST_NAME
...
async def cust_add_addr(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await cust_add_save(update, context); return ConversationHandler.END
...
async def cust_add_save(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    get_db(context).add(c); get_db(context).commit()
...
async def report_today(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    from bot.keyboards import back_to_menu
    await query.edit_message_text(f"📅 *مبيعات اليوم ({today})*\n\nعدد الفواتير: {len(invoices)}\nالإجمالي: {total_sales:,.0f} د.ع",
...
async def report_month(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    total_sales = sum(i.grand_total for i in invoices)
    from bot.keyboards import back_to_menu
    await query.edit_message_text(f"📆 *مبيعات شهر {today.strftime('%m/%Y')}*\n\nعدد الفواتير: {len(invoices)}\nالإجمالي: {total_sales:,.0f} د.ع",
        reply_markup=back_to_menu())
...
async def report_low_stock(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not products: text = "✅ لا توجد منتجات منخفضة المخزون."
    else: text = "⚠️ *المنتجات منخفضة المخزون:*\n\n" + "".join(f"• {p.name}: {p.current_stock} / {p.min_stock}\n" for p in products)
    from bot.keyboards import back_to_menu
    await query.edit_message_text(text, reply_markup=back_to_menu())
...
async def setting_edit(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not s: await query.edit_message_text("الإعداد غير موجود."); return
...
async def setting_save(update: Update, context: ContextTypes.DEFAULT_TYPE):
    val = update.message.text; sid = context.user_data.get("edit_setting_id")
    key = context.user_data.get("edit_setting_key")
    if sid is None: await update.message.reply_text("خطأ."); return ConversationHandler.END
    db = get_db(context)
    s = db.query(Setting).filter(Setting.id == sid).first()
    if s: s.value = val; db.commit()
    context.user_data.pop("edit_setting_id", None); context.user_data.pop("edit_setting_key", None)
    from bot.keyboards import back_to_menu
...
async def handle_skip(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    action = query.data
    return {
        "skip_image": prod_add_image_skip,
        "skip_sizes": prod_add_sizes_skip,
        "skip_colors": prod_add_colors_skip,
        "skip_prints": prod_add_prints_skip,
        "skip_phone": cust_add_phone_skip,
        "skip_addr": cust_add_addr_skip,
    }.get(action, lambda u, c: ConversationHandler.END)(update, context)
...
def get_handlers():
    login_conv = ConversationHandler(
        ...
        fallbacks=[CallbackQueryHandler(login_cancel, pattern="^cancel$")],
        per_message=False,
    )
    ...
    edit_setting_conv = ConversationHandler(
        ...
        states={EDIT_VALUE: [MessageHandler(filters.TEXT & ~filters.COMMAND, setting_save)]},
