...
def _get_empty_rows(items):
    """Fill remaining rows up to 12 total for consistent look"""
    count = len(items)
    rows = ""
...
def preview_invoice(invoice: dict, store: dict, parent=None):
    html = build_invoice_html(invoice, store)
    tmp = tempfile.NamedTemporaryFile(mode="w", suffix=".html", delete=False, encoding="utf-8")
    tmp.write(html)
    tmp_path = tmp.name
    tmp.close()
    webbrowser.open(f"file:///{tmp_path.replace(chr(92), '/')}")
    QMessageBox.information(parent, "طباعة", "الفاتورة مفتوحة في المتصفح.\nاضغط Ctrl+P للطباعة.")


def print_invoice(invoice: dict, store: dict, parent=None):
    preview_invoice(invoice, store, parent)
